...
pub fn discover_registration_file(config: &LicenseConfig) -> Result<RegistrationFileDiscovery, LicenseError> {
    ...
    for candidate in candidate_paths(config) {
        ...
        let bytes = fs::read(&candidate).map_err(|e| LicenseError::RegistrationFile(e.to_string()))?;
        ...
        let verified = match (&payload, &config.registration_public_key_base64) {
            (Some(payload), Some(pub_key)) if !pub_key.trim().is_empty() => Some(verify_signature(payload, pub_key)?),
            _ => None,
        };
...
fn candidate_paths(config: &LicenseConfig) -> Vec<PathBuf> {
    ...
    if let Ok(program_data) = env::var("PROGRAMDATA") {
        roots.push(PathBuf::from(program_data).join("WWSoftwares").join("IntegraDesktop"));
    }
...
fn verify_signature(payload: &Value, public_key_b64: &str) -> Result<bool, LicenseError> {
    let obj = payload
        .as_object()
        .ok_or_else(|| LicenseError::RegistrationFile("payload do arquivo de registro inválido".to_string()))?;

    let signature_value = obj
        .get("signature")
        .and_then(|v| v.get("value"))
        .and_then(|v| v.as_str())
        .ok_or_else(|| LicenseError::RegistrationFile("assinatura não encontrada no arquivo de registro".to_string()))?;
    ...
    let public_key_array: [u8; 32] = public_key_bytes
        .try_into()
        .map_err(|_| LicenseError::RegistrationFile("a chave pública Ed25519 deve conter 32 bytes".to_string()))?;
    let signature_array: [u8; 64] = signature_bytes
        .try_into()
        .map_err(|_| LicenseError::RegistrationFile("a assinatura Ed25519 deve conter 64 bytes".to_string()))?;
