...
pub fn map_document_to_ba_prestados_line(document: &NfseDocument, profile: &ConversionProfile) -> Result<String> {
    let len = 1172usize;
    let mut chars = vec![' '; len];

    let serie = sanitize_serie(&document.serie);
    let numero = only_digits(&document.numero);
    let service_code = apply_string_rule(&document.item_lista_servico, &profile.field_rules.codigo_servico);
    let code_4 = if service_code.len() > 4 { service_code[..4].to_string() } else { format!("{:0>4}", service_code) };
    let code_12 = if service_code.len() > 4 { service_code.clone() } else { String::new() };
    let observacao_base = first_non_empty(vec![document.discriminacao.clone(), document.info_adic.clone()]);
    let observacao = apply_string_rule(&observacao_base, &profile.field_rules.observacao);
    let observacao_curta = observacao.chars().take(40).collect::<String>();
    let municipio_nome = sanitize_text(&document.municipio_nome).chars().take(20).collect::<String>();
    let municipio_codigo = only_digits(&document.municipio_codigo);
    let tomador_doc = only_digits(&document.tomador.documento);

    let base_calculo = apply_number_rule(document.taxes.base_calculo, &profile.field_rules.base_calculo);
    let aliquota = apply_number_rule(document.taxes.aliquota_iss, &profile.field_rules.iss_aliquota);
    let valor_iss = apply_number_rule(document.taxes.valor_iss, &profile.field_rules.iss_devido);
    ...
    let valor_cofins = apply_number_rule(document.taxes.valor_cofins, &profile.field_rules.valor_cofins);
    let _valor_csll = apply_number_rule(document.taxes.valor_csll, &profile.field_rules.valor_csll);
    let _valor_liquido = apply_number_rule(document.taxes.valor_liquido, &profile.field_rules.valor_liquido);
    let iss_retido = apply_bool_rule(document.taxes.iss_retido, &profile.field_rules.iss_retido);

    put(&mut chars, 1, 1, "1", false, ' ');
    put(&mut chars, 2, 6, &ddmmaa(&document.emissao), true, ' ');
    put(&mut chars, 8, 5, &serie, false, ' ');
    put(&mut chars, 31, 4, &code_4, true, ' ');
    put(&mut chars, 35, 1, "0", false, ' ');
    put(&mut chars, 36, 14, &format_money(document.taxes.valor_servicos), true, ' ');
    ...
    put(&mut chars, 308, 1, if iss_retido { "R" } else { "P" }, false, ' ');
    put(&mut chars, 538, 1, &profile.responsavel_retencao, false, ' ');
    ...
    if iss_retido {
        ...
        put(&mut chars, 617, 8, &ddmmaaaa(&document.emissao), false, ' ');
    }
    ...
    put(&mut chars, 644, 2, &profile.operacao_nota, false, ' ');
    put(&mut chars, 646, 3, &profile.tipo_recolhimento, false, ' ');
    put(&mut chars, 650, 3, &profile.tipo_documento, false, ' ');
    put(&mut chars, 654, 5, &municipio_codigo.chars().take(5).collect::<String>(), false, ' ');
    put(&mut chars, 659, 10, &profile.situacao_documento, false, ' ');
    put(&mut chars, 669, 5, &profile.especie_documento, false, ' ');
    ...
    put(&mut chars, 694, 8, &format_rate_from_tax(valor_pis, base_calculo), true, ' ');
    put(&mut chars, 702, 14, &format_money(valor_pis), true, ' ');
    put(&mut chars, 716, 8, &format_rate_from_tax(valor_cofins, base_calculo), true, ' ');
    ...
    put(&mut chars, 910, 1, if should_emit_obs(&observacao, &profile.obs_extended) { "1" } else { "0" }, false, ' ');
...
fn apply_bool_rule(source: bool, rule: &FieldRule) -> bool {
    match rule.action {
        ...
        FieldAction::Constant => matches!(rule.value.as_deref(), Some("1") | Some("true") | Some("TRUE") | Some("S") | Some("s")),
    }
}
...
fn fit_left(value: &str, size: usize) -> String {
    ...
    if digits.len() >= size { digits[digits.len() - size..].to_string() } else { digits }
}
...
fn format_money(value: f64) -> String { format!("{value:.2}") }
fn format_money_short(value: f64) -> String { format!("{value:.2}") }
fn format_money_4(value: f64) -> String { format!("{value:.4}") }
...
fn sanitize_serie(value: &str) -> String {
    value.chars().filter(|c| c.is_ascii_alphanumeric()).take(5).collect::<String>().to_uppercase()
}
