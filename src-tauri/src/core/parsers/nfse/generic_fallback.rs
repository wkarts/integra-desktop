...
use super::abrasf_v2::{find_first, find_first_from, text_from_descendant, parse_decimal, only_digits, sanitize, normalize_service_code, extract_chave, resolve_municipio, option_string, optional_digits, first_non_empty, fallback_text};
...
pub fn parse_ubaira(xml: &str, file_name: &str) -> Result<NfseDocument> {
    ...
    let info_adic = first_non_empty(vec![text_from_descendant(inf, "OutrasInformacoes"), text_from_descendant(inf, "CodigoVerificacao")]);
    let municipio_raw = first_non_empty(vec![text_from_descendant(inf, "MunicipioIncidencia"), text_from_descendant(inf, "CodigoMunicipio")]);
