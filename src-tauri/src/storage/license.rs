...
use crate::core::domain::license::{LicenseSettings, LicenseSnapshot};
...
pub fn save_license_snapshot(app: &AppHandle, snapshot: &LicenseSnapshot) -> Result<()> {
    let file = snapshot_file(app)?;
    if let Some(parent) = file.parent() {
        fs::create_dir_all(parent)?;
    }
...
pub fn load_license_snapshot(app: &AppHandle) -> Result<Option<LicenseSnapshot>> {
    let file = snapshot_file(app)?;
    if !file.exists() {
        return Ok(None);
    }
...
fn snapshot_file(app: &AppHandle) -> Result<PathBuf> {
    let dir = app.path().app_data_dir()?;
    Ok(dir.join("cache").join("license_snapshot.json"))
}
